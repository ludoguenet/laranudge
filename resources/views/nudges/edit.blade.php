            ...
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <section>
                        ...
                        <form method="post" action="{{ route('admin.nudges.update', $nudge) }}" class="mt-6 space-y-6">
                            @csrf
                            @method('put')

                            <div>
                                <x-input-label for="title" :value="__('Title')" />
                                <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title', $nudge->title)" autofocus autocomplete="title" required />
                                <x-input-error class="mt-2" :messages="$errors->get('title')" />
                            </div>

                            <div>
                                <x-input-label for="content" :value="__('Short description')" />
                                <textarea rows="7" name="content" id="content" class="block w-full rounded-md border-0 py-1.5 mt-1 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-green-600 sm:text-sm sm:leading-6" placeholder="Your description goes here!">{{ old('content', $nudge->content) }}</textarea>
                            ...
                            <div x-data="tabs">
                                ...
                                <div class="mt-2">
                                    <div x-show="currentTab === 1" id="tabs-1-panel-1" class="-m-0.5 rounded-lg p-0.5" aria-labelledby="tabs-1-tab-1" role="tabpanel" tabindex="0">
                                        ...
                                        <div>
                                            <textarea rows="7" name="code" id="code" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-green-600 sm:text-sm sm:leading-6" placeholder="Your PHP snippet goes here!">{{ old('code', $nudge->code) }}</textarea>
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 4000)" class="text-sm text-gray-600">{{ __('Nudge edited! Awaiting validation again. Almost there!') }}</p>
                                @endif
                            </div>
                        </form>
                    </section>
